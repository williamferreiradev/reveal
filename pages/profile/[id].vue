...
<template>
    ...
    <div v-else class="min-h-screen bg-gray-50 dark:bg-background">
        <!-- Orange Banner -->
        <div class="h-60 bg-gradient-to-r from-orange-400 to-orange-600 w-full relative">
        ...
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
            <div class="flex flex-col lg:flex-row gap-8 -mt-20">
                ...
                <div class="w-full lg:w-80 flex-shrink-0">
                    <div class="bg-white dark:bg-surface rounded-2xl p-6 shadow-xl border border-gray-100 dark:border-gray-800">
                        ...
                        <div class="flex justify-between items-center text-center border-t border-b border-gray-100 dark:border-gray-800 py-4 mb-6">
                            <div>
                                <div class="font-bold text-gray-900 dark:text-white text-lg">0</div>
                                <div class="text-xs text-gray-500 uppercase tracking-wide">Seguidores</div>
                            </div>
                            <div>
                                <div class="font-bold text-gray-900 dark:text-white text-lg">0</div>
                                <div class="text-xs text-gray-500 uppercase tracking-wide">Seguindo</div>
                            </div>
                            <div>
                ...
                <div class="flex-1 pt-4 lg:pt-20">
                    <!-- Tabs -->
                    <div class="flex items-center gap-8 border-b border-gray-200 dark:border-gray-800 mb-8 overflow-x-auto">
                        <button 
                            @click="activeTab = 'showcase'"
                            class="pb-3 text-sm font-medium transition-colors border-b-2 whitespace-nowrap"
                            :class="activeTab === 'showcase' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400'"
                        >
                            Posts Criados
                        </button>
                        <button 
                            ...
                            class="pb-3 text-sm font-medium transition-colors border-b-2 whitespace-nowrap"
                            :class="activeTab === 'likes' ? 'border-primary text-primary' : 'border-transparent text-gray-500 hover:text-gray-700 dark:text-gray-400'"
                        >
                            Curtidas
                        </button>
                    </div>
                    ...
                    <!-- Grid -->
                    <div v-if="activeTab === 'showcase'" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        <div v-for="post in posts" :key="post.id" class="group relative aspect-square bg-gray-100 dark:bg-surface rounded-xl overflow-hidden shadow-sm hover:shadow-md transition-all">
                            ...
                            <!-- Image/Video -->
                            <img 
                                v-if="post.conteudo_url && post.tipo === 'post'" 
                                :src="post.conteudo_url" 
                                class="w-full h-full object-cover"
                            />
                            <div v-else-if="post.tipo === 'video'" class="w-full h-full flex items-center justify-center bg-gray-900 text-white">
                                <span class="text-xs uppercase font-bold tracking-widest">Vídeo</span>
                            </div>
                            <div v-else class="w-full h-full p-4 flex items-center justify-center text-center text-gray-500 text-sm">
                                {{ post.descricao || 'Sem conteúdo visual' }}
                            </div>
                            ...
                            <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-4 text-white">
                                <div class="flex items-center gap-1">
                                    ...
                                    <span class="font-bold">{{ post.curtidas_count || 0 }}</span>
                                </div>
                                <div class="flex items-center gap-1">
                                    <ImageIcon :size="18" />
                                </div>
                    ...
                    <div v-else class="py-12 text-center text-gray-500">
                        Histórico de curtidas não disponível publicamente.
                    </div>
